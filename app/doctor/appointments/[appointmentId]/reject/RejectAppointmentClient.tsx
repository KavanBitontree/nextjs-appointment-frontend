...
export default function RejectAppointmentClient() {
  ...
  useEffect(() => {
    ...
    const reject = async () => {
      try {
        ...
        setMessage(err?.response?.data?.detail || "Unable to reject appointment");
        setStatus("error");
      }
    };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow p-6 text-center">
        ...
        {status === "error" && (
          <>
            <h1 className="text-2xl font-bold text-red-600">⚠️ Action Failed</h1>
            <p className="mt-4 text-gray-700">{message}</p>
