...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${process.env.NEXT_PUBLIC_API_URL}/doctors`);
    ...
    if (search_address)
      backendUrl.searchParams.set("search_address", search_address);
    if (filter_speciality)
      backendUrl.searchParams.set("filter_speciality", filter_speciality);
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: errorData.detail || "Failed to fetch doctors",
        }),
        {
